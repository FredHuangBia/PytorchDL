...
def findData(opt):
	...
	for i, line in enumerate([itm for itm in xmlRaw]):
		...
		dataPaths.append(os.path.join('./',pieces[0]))
...
def mergeData(dataPath, opt):
	for i in range(opt.numEntry):
		...
		if not os.path.exists(mergedName):
			...
			dataRaw.close()
			pieces = lines[0].split() # suppose only one line
			for dim in range(opt.dataSize[0]):
				merged[dim] = float(pieces[dim])
